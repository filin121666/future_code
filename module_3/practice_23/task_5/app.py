...
def create_user():
    ...
    req = requests.post(url='http://127.0.0.1:5001/create_user_api', json={'username': username, 'password': password})
    data: dict = req.json()
...
def delete_user():
    ...
    req = requests.post(url='http://127.0.0.1:5001/delete_user_api', json={'username': username, 'password': password})
    data: dict = req.json()
    if data.get('status_code') == 200:
